...
from fourier_propagation import spatial_grid_1d, gaussian_1d, thin_lens_phase_1d
from fourier_propagation import fresnel_propagate_fft_1d

lam = 500e-9
dx  = lam/2
Nx  = 4096                         # ensures z_max = 0.512 mm without Fresnel aliasing
x   = spatial_grid_1d(Nx, dx)

z_max  = Nx*dx*dx/lam              # = N dx^2 / λ
z_lens = z_max/2
f      = z_max/4                   # 2f-2f imaging with lens at midplane
w0     = 1.5*lam

U0 = gaussian_1d(Nx, dx, w0, lam, z=0.0)
# propagate to lens, apply lens, then to output
...
plt.title('Gaussian + thin lens (focus at z_max): input vs output')
